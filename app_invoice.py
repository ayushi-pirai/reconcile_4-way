...
if 'invoice_texts' not in st.session_state:
    st.session_state.invoice_texts = {}
...
def parse_invoice_text(text):
    ...
    # 5) Amount ($ only): look for labeled totals first
    amount_patterns = [
    ...
    # 6) Description: try label; else empty
...
def extract_invoices_with_text(invoice_files):
    records = []
    texts = {}
    for invoice_id_key, file_path in invoice_files.items():
        try:
...
if page == "Upload Data":
    ...
    if use_sample_data:
        ...
        if OCR_AVAILABLE:
            try:
                ocr_invoice_df, ocr_texts = extract_invoices_with_text(st.session_state.invoice_files)
                st.session_state.invoice_df = ocr_invoice_df
                st.session_state.invoice_texts = ocr_texts
        ...
        # OCR debug view
        if 'invoice_df' in st.session_state and len(st.session_state.invoice_df) > 0:
            st.markdown("---")
            st.subheader("OCR Extracted Invoices (from PDFs)")
            st.dataframe(st.session_state.invoice_df)
            inv_ids = st.session_state.invoice_df['Invoice_ID'].tolist()
            sel_id = st.selectbox("Select an invoice to view raw OCR text", [""] + inv_ids)
            if sel_id:
                texts = st.session_state.get('invoice_texts', {})
                st.text_area("Raw OCR Text", texts.get(sel_id, ''), height=300)
        ...
        if uploaded_ledger and uploaded_bank and (uploaded_invoices or uploaded_invoice_files):
            ...
            # Normalize ledger to ensure Vendor exists (for display only)
            ...
            if uploaded_invoice_files:
                ...
                if OCR_AVAILABLE:
                    try:
                        ocr_invoice_df, ocr_texts = extract_invoices_with_text(invoice_files)
                        st.session_state.invoice_df = ocr_invoice_df
                        st.session_state.invoice_texts = ocr_texts
        ...
        # OCR debug view for uploaded/directory data
        if 'invoice_df' in st.session_state and len(st.session_state.invoice_df) > 0:
            st.markdown("---")
            st.subheader("OCR Extracted Invoices (from PDFs)")
            st.dataframe(st.session_state.invoice_df)
            inv_ids = st.session_state.invoice_df['Invoice_ID'].tolist()
            sel_id = st.selectbox("Select an invoice to view raw OCR text", [""] + inv_ids, key="ocr_debug_select")
            if sel_id:
                texts = st.session_state.get('invoice_texts', {})
                st.text_area("Raw OCR Text", texts.get(sel_id, ''), height=300)
    ...
    if 'processed' not in st.session_state:
        ...
        # Description analysis (replaces Vendor analysis)
        st.subheader("Top Descriptions by Amount")
        desc_totals = ledger_result.groupby('Description')['Amount'].sum().reset_index()
        desc_totals = desc_totals.sort_values('Amount', ascending=False)
        fig = px.bar(desc_totals.head(10), x='Description', y='Amount', title='Top 10 Descriptions by Amount')
        st.plotly_chart(fig, use_container_width=True)
        ...
        if st.button("Generate Report"):
            with st.spinner("Generating report..."):
                ...
                Top Descriptions by Amount:
                """
                
                for _, row in desc_totals.head(5).iterrows():
                    report_content += f"- {row['Description']}: ${row['Amount']:.2f}\n"
